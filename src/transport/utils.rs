...
mod tests{
    ...
    #[test]
    fn get_ports_mine(){
        let start = SystemTime::now().duration_since(UNIX_EPOCH).expect("should be able to get time");
        let s = Instant::now();
        let (t,p) = find_available_port_mine()
        ...
        let end = SystemTime::now().duration_since(UNIX_EPOCH).expect("should be able to get time");
        let e = Instant::now();
